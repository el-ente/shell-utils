...
repo() {
    if [ -z "${REPOSITORIES_FOLDER:-}" ]; then
        echo "Error: REPOSITORIES_FOLDER environment variable is not set" >&2
        return 1
    fi
    if [ ! -d "$REPOSITORIES_FOLDER" ]; then
        echo "Error: REPOSITORIES_FOLDER directory does not exist: $REPOSITORIES_FOLDER" >&2
        return 1
    fi
    local selected
